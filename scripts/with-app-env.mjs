...
function main(argv) {
  ...
  // On Windows, locally installed CLIs (vite, tsc, …) resolve to a `.cmd`
  // shim; `spawn()` only runs those through a shell, otherwise it fails with
  // ENOENT even though the executable is right there in `node_modules/.bin`.
